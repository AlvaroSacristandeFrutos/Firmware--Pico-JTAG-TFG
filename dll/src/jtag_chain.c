...
int jtag_scan_chain(JLINKARM_JTAG_IDCODE_INFO *pInfo, int maxDev) {
    ...
    /* Precargar con 1s hasta JTAG_MAX_IR_TOTAL_BITS (4096 bits = 512 bytes).
     * Versión anterior usaba solo 1024 bits; si la cadena tiene >1024 bits de IR
     * total (posible con 32 dispositivos × 32+ bits cada uno), las posiciones más
     * alejadas de TDO no quedaban a 1 y el algoritmo de detección fallaba. */
    uint8_t ones[JTAG_MAX_IR_TOTAL_BITS / 8u];   /* 512 bytes en stack — tamaño razonable para DLL Windows */
    memset(ones, 0xFF, sizeof(ones));
    jtag_shift_data(ones, NULL, JTAG_MAX_IR_TOTAL_BITS, false);
